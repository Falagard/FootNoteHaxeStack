...
class MenuEditor extends Dialog {
    ...
    function buildUI():Void {
        ...
        var nameLbl = new Label(); nameLbl.text = "Name:"; nameLbl.width = 100; nameRow.addComponent(nameLbl);
        nameField = new TextField(); nameField.percentWidth = 100; nameRow.addComponent(nameField);
        root.addComponent(nameRow);

        var slugRow = new HBox();
        var slugLbl = new Label(); slugLbl.text = "Slug:"; slugLbl.width = 100; slugRow.addComponent(slugLbl);
        slugField = new TextField(); slugField.percentWidth = 100; slugRow.addComponent(slugField);
        ...
        var spacer = new haxe.ui.components.Spacer(); spacer.percentWidth = 100; btnRow.addComponent(spacer);
        saveBtn = new Button(); saveBtn.text = "Save"; saveBtn.onClick = function(_) save(); btnRow.addComponent(saveBtn);
        closeBtn = new Button(); closeBtn.text = "Close"; closeBtn.onClick = function(_) this.hideDialog(null); btnRow.addComponent(closeBtn);
    ...
    function save():Void {
        ...
        untyped services.AsyncServiceRegistry.instance.megaMenu.updateMenuAsync(menu.id, menu, function(success:Bool) {
            if (success) {
                if (onSaved != null) onSaved(menu);
