...
class LoginDialog extends Dialog {
	...
	public function new() {
		...
		if (loginBtn != null)
			loginBtn.onClick = function(_) doLogin();
		if (registerBtn != null)
			registerBtn.onClick = function(_) showRegister();
		if (cancelBtn != null)
	...
	function validate(emailOrUsername:String, password:String):Array<String> {
		...
		if (emailOrUsername.length == 0)
			errs.push("Email/username required");
	...
	function doLogin():Void {
		...
		if (loggingInRow != null)
			loggingInRow.hidden = false;
